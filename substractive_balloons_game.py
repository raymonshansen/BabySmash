...
class InGameBallon:
    ...
    def draw(self, screen):
        self.image = self.pop_imgs[0]
        if self.popping and self.img_index<len(self.pop_imgs):
...
class SubstractiveBalloons:
    state_name = "SUBSTRACTIVEBALLOONS"
    config_state_name = "SUBSTRACTIVE_BALLONS_CONFIG"
    main_menu_name = "Substractive Balloons"
    ...
    def generate_balloons(self, num):
        ...
        for n in range(0,num):
            ...
            while firstNumber < secondNumber:
                secondNumber = random.randint(0,10)
            differences.append('%d-%d' % (firstNumber,secondNumber))
        ret = list()
        for i in range(num):
            ba = self.new_balloon(differences[i])
